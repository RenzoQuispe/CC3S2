...
class InfrastructureBuilder:
    ...
    def build_welcome_resource(self) -> "InfrastructureBuilder":
        
        # Crea un recurso basado en un prototipo null_resource y añade, mediante un mutator, un bloque local_file que genera el archivo 'bienvenida.txt' al aplicar Terraform.
    ...
    def build_group(self, name: str, size: int) -> "InfrastructureBuilder":
        ...
        for i in range(size):
            def mut(block: Dict[str, Any], idx=i) -> None:
                ...
                original_name = next(iter(res_block.keys()))
                res = res_block.pop(original_name)
                res_block[f"{name}_{idx}"] = res
                res_block[f"{name}_{idx}"][0]["triggers"]["group_index"] = idx
            
            self._module.add(proto.clone(mut).data)
